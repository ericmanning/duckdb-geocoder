// Vendored from splink_udfs (MIT) — copyright (c) 2025 Ministry of Justice.
// Source: https://github.com/moj-analytical-services/splink_udfs/blob/main/src/include/phonetic/soundex.hpp
// Original attribution preserved below per MIT terms.
//
// We vendor this single file rather than depend on splink_udfs at runtime —
// soundex is the only function we need from it, and avoiding the community
// dep makes us_geocoder install cleanly with just `INSTALL us_geocoder FROM
// community; LOAD us_geocoder;` (plus spatial). See LICENSE-vendored at the
// repo root for the full MIT notice.
//
// -------------------------------------------------------------------------
...
inline const char *Soundex::Encode(const char *str) {
	...
	for (; *str && out_idx < length_; ++str) {
		...
		// Explicitly ignore non-ASCII bytes and non-letters.
		if (current_byte > 127 || !std::isalpha(current_byte)) {
			continue;
		}
