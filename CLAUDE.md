...
- **In-process parallel download (May 2026, `parallel := true` default).** The shell-script speed-up moved into the loader: each state's zips download concurrently into a temp dir via DuckDB's `httpfs` HTTP client, ingest locally, then the temp dir is wiped before the next state. Benchmarked NJ end-to-end at **58 s** (vs 116 s on the legacy `/vsicurl/` path — 2× wall-clock win, same as the shell script, no shell deps). Peak disk bounded at `max(state_size)` regardless of `load_tiger_all_states` size. Also fixes the Windows MSVC failure where GDAL's bundled curl couldn't validate the Census cert chain on `/vsizip/{/vsicurl/https://...}` — `httpfs` uses mbedTLS with bundled CAs. `parallel := false` reverts to the legacy `/vsicurl/` path for A/B benchmarking.
...
- ~~Real parallel HTTPS loads at the loader layer~~ — **shipped May 2026** as the `parallel := true` default in the C++ loader; see Loader performance § above.
...
- ~~All-states pre-download → local-ingest → cleanup wrapper~~ — **shipped May 2026.** `CALL load_tiger_all_states();` now does exactly this in-process (parallel download per state, ingest, `RemoveDirectory`, next state) via the `parallel := true` default. [`scripts/parallel_download_state.sh`](scripts/parallel_download_state.sh) is deprecated, scheduled for removal in the next minor release.
