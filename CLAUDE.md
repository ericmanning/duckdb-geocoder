...
- **v0.2: per-state TIGER tables (storage sharding).** Replace the unified `tiger.<table>` shape with one table per `statefp` (e.g. `tiger.edges_05`, …, ×50 for each of 13 data tables = ~663 base tables). Considered during the May 2026 batch-perf work (`perf/geocode-batch-planning`); deferred because the C++ batch table function over unified tables + `WHERE statefp = '<lit>'` (with ART + zone-map pruning) closed the immediate perf gap without the refactor cost. Sharding would buy us marginal per-query overhead reduction (~10–50ms each) and better OS page-cache locality for repeated same-state queries. The architectural-alignment argument with eventual parquet partitioning was weaker than I thought (DuckDB's parquet partition pruning isn't as aggressive as Spark's). Re-evaluate when the C++ batch path's per-query plan overhead becomes the dominant cost in profiling.
