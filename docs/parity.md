...
**Two "wrongness" categories now (was three — #1145d reclassified after deeper trace):**
...
- **D7 cost (4 tests)**: #1076h, #1113d, #1073a, #1145d — all four trace back to PG's primary/fallback stage_a structure that we deliberately collapsed in D7. Closeable only by D7 reversal (separate planned work). Until then, all four divergences are expected consequences.

**Net: no remaining bugs in our code.** All 8 divergences are accounted for by deliberate design choices or by D7. When D7 is revisited (as planned), the four D7-cost cases will all be re-evaluated — porting PG's primary/fallback split should resolve most or all of them. The four us-better-than-PG cases need careful handling at that time so they don't regress.
