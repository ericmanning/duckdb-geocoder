...
- Bump versions in `.github/workflows/MainDistributionPipeline.yml`
  - `duckdb_version` and `ci_tools_version` inputs on the `duckdb-build` job
  - `duckdb_version` and `ci_tools_version` inputs on the `code-quality-check` job
  - the `@vX.Y.Z` ref on each `uses: duckdb/extension-ci-tools/...` line
